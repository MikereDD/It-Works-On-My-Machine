...
import com.typezero.cloudtv.data.MediaItem
...
private fun VlcPlayer(
    queue: List<com.typezero.cloudtv.data.MediaItem>,
    resolveUrl: suspend (com.typezero.cloudtv.data.MediaItem) -> ApiResult<String>,
    startIndex: Int,
    playlistKey: String? = null,
    onExit: () -> Unit
) {
    val context = LocalContext.current
    val store = remember { com.typezero.cloudtv.data.SessionStore(context) }
    ...
    val cast = com.typezero.cloudtv.cast.rememberCastController()
    ...
    DisposableEffect(isPlaying, hasVideo, cast.isCasting) {
        ...
        if (isPlaying && !cast.isCasting) {
            ...
            if (hasVideo) {
                ...
                com.typezero.cloudtv.playback.PlaybackService.stop(context)
            } else {
                window?.clearFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)
                // Audio: keep the process alive so playback continues in the background.
                com.typezero.cloudtv.playback.PlaybackService.start(context, title)
            }
        } else {
    ...
    Box(
        ...
        if (cast.isAvailable) {
            androidx.compose.animation.AnimatedVisibility(
                ...
                com.typezero.cloudtv.cast.CastButton(
